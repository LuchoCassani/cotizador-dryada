...
import type { BuildVolume } from '../../src/services/prusa-slicer.service';

const mockSpawn   = vi.mocked(spawn);
const mockRead    = vi.mocked(fs.readFile);
const mockUnlink  = vi.mocked(fs.unlink);
const BUILD_VOLUME: BuildVolume = { xMm: 300, yMm: 300, zMm: 300 };
...
describe('PrusaSlicerService', () => {
  ...
  it('caso éxito: devuelve gramosTotal parseado del gcode', async () => {
    ...
    const result = await svc.slice('/tmp/abc123.stl', 1.24, BUILD_VOLUME);
  ...
  it('caso éxito: llama al binario con los flags correctos', async () => {
    ...
    setImmediate(() => proc.emit('close', 0));
    await svc.slice('/tmp/abc123.stl', 1.24, BUILD_VOLUME);
  ...
  it('usa las dimensiones de buildVolume para --bed-shape y --max-print-height', async () => {
    const proc = makeProc();
    mockSpawn.mockReturnValue(proc as any);
    mockRead.mockResolvedValue(GCODE_OK as any);

    setImmediate(() => proc.emit('close', 0));
    await svc.slice('/tmp/abc123.stl', 1.24, { xMm: 300, yMm: 300, zMm: 600 });

    expect(mockSpawn).toHaveBeenCalledWith(
      'prusa-slicer',
      expect.arrayContaining(['--bed-shape', '0x0,300x0,300x300,0x300', '--max-print-height', '600']),
      expect.any(Object),
    );
  });
  ...
  it('EC-001: lanza cuando el binario no existe (ENOENT)', async () => {
    ...
    await expect(svc.slice('/tmp/abc123.stl', 1.24, BUILD_VOLUME)).rejects.toThrow();
  });
  ...
  it('EC-002: lanza cuando PrusaSlicer sale con exit code distinto de 0', async () => {
    ...
    await expect(svc.slice('/tmp/abc123.stl', 1.24, BUILD_VOLUME)).rejects.toThrow('exit code 1');
  });
  ...
  it('EC-003: lanza y mata el proceso cuando supera el timeout configurado', async () => {
    ...
    const slicePromise = shortTimeoutSvc.slice('/tmp/abc123.stl', 1.24, BUILD_VOLUME);
  ...
  it('EC-004: lanza cuando el gcode no contiene la línea filament used [g]', async () => {
    ...
    await expect(svc.slice('/tmp/abc123.stl', 1.24, BUILD_VOLUME)).rejects.toThrow('parse failed');
  });
  ...
  it('EC-005: lanza cuando el valor parseado es 0', async () => {
    ...
    await expect(svc.slice('/tmp/abc123.stl', 1.24, BUILD_VOLUME)).rejects.toThrow('invalid value');
  });
  ...
  it('progreso: parsea líneas "N => etapa" del stdout y llama a onProgress', async () => {
    ...
    await svc.slice('/tmp/abc123.stl', 1.24, BUILD_VOLUME, undefined, onProgress);
  ...
  it('EC-002b: el error de exit code distinto de 0 incluye el tail de stderr para diagnóstico', async () => {
    ...
    await expect(svc.slice('/tmp/abc123.stl', 1.24, BUILD_VOLUME)).rejects.toThrow('error: Unable to repair mesh');
  });
  ...
  it('progreso: onProgress es opcional, no rompe el flujo si se omite', async () => {
    ...
    await expect(svc.slice('/tmp/abc123.stl', 1.24, BUILD_VOLUME)).resolves.toMatchObject({ gramosTotal: 12.45 });
  });
});
